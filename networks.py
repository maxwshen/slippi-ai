...
class LSTM(snt.Module):
  ...
  def partial_restart_state(self, restart_mask, keep_mask):
    ...
    return LSTMState(
      hidden = tf.multiply(self.lstm_state.hidden, restart_mask) + \
        tf.multiply(self.init_state.hidden, keep_mask),
      cell = tf.multiply(self.lstm_state.cell, restart_mask) + \
  ...
  def __call__(self, data, restarting):
    ...
    for t in tf.range(data.shape[0]):
      if tf.reduce_any(restarting[t]):
        self.lstm_state = self.partial_restart_state(rs_mat[t], not_rs_mat[t])

      out_t, self.lstm_state = self.network(data[t], self.lstm_state)
